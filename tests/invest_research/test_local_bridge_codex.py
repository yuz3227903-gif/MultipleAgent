...
class TestTranslatingEvents:
    ...
    def test_current_codex_item_completion_becomes_assistant_text(self):
        events = translate({
            "type": "item.completed",
            "item": {"id": "item_1", "type": "agent_message", "text": "连接成功"},
        })
        assert kinds(events) == ["message.delta"]
        assert events[0].data["text"] == "连接成功"
...
class TestToleratingVersionDrift:
    ...
    def test_a_known_codex_startup_warning_is_not_shown_as_an_answer(self):
        assert translate("2026-08-24T04:48:46Z WARN codex_core::shell_snapshot: unsupported") == []

    def test_the_skill_budget_advisory_is_not_shown_as_a_failure(self):
        assert translate({
            "type": "item.completed",
            "item": {
                "type": "error",
...
class TestResuming:
    ...
    def test_current_codex_thread_id_is_captured_for_the_next_turn(self):
        events = translate({"type": "thread.started", "thread_id": "thread-42"})
        assert CodexAdapter().session_reference(events) == "thread-42"

...
class CodexAdapter(LocalAgentAdapter):
    ...
    def run_args(self, *, session_ref: str | None) -> list[str]:
        ...
        if session_ref:
            # Codex 0.149+ resumes through the `exec resume` subcommand.
            return ["exec", "resume", "--json", "--skip-git-repo-check", session_ref]
    ...
    def _events_for(self, kind: str, body: dict[str, Any], session_id: str) -> list[AgentEvent]:
        if not kind:
            return []
        if "session" in kind or kind == "thread.started":
            ...
            reference = body.get("session_id") or body.get("thread_id") or body.get("id")
            if not reference:
                return []
        ...
        if "error" in kind:
            ...
            if message.startswith("Skill descriptions were shortened to fit the skills context budget"):
                return []

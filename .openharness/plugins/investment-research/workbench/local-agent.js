...
async function sendToLocalAgent(agent, text) {
  ...
  const agentName = agent.name || agent.agent_id || '本地 Agent';
  const avatar = agentName.trim().slice(0, 2).toUpperCase();
  appendActivity(`<div class="local-turn">
    <div class="local-message local-message-user"><span class="local-avatar local-avatar-user">你</span><div><strong>你</strong><p>${esc(text)}</p></div></div>
    <div class="local-message local-message-agent"><span class="local-avatar">${esc(avatar)}</span><div><strong>${esc(agentName)} <em>本地 Agent</em></strong><pre id="local-answer" class="local-answer" aria-live="polite"></pre></div></div>
  </div>`);
